  ...
  template: `
    <div style="position:relative" class="{{ zoomClass }}">
      ...
      <ng-template [ngIf]="!disabled">
        <div
          class="text-sm-left buttonGroup signatureCtrls"
        ...
        >
          ...
          <button
            ...
            *ngIf="zoomFactor == 1"
          >
            <i class="fa fa-fw fa-plus-square" aria-hidden="true"></i>Zoom In
          </button>
          <button
            class="btn btn-plain"
            (click)="shrinkCanvas()"
            *ngIf="zoomFactor == 2"
          >
            <i class="fa fa-fw fa-minus-square" aria-hidden="true"></i>Zoom Out
          </button>
          <button
            *ngIf="element.config?.uiStyles?.attributes?.scriptName"
            (click)="getUpdatedSignature()"
            type="button"
            class="btn btn-plain post-btn"
          >
...
export class Signature extends BaseControl<string> {
  ...
  defaultEmptyImage: string =
    'data:image/png;base64,';
  height: number;
  ...
  private renderExistingSignature() {
    if (this.value) {
      this.logger.debug(
        `Found exsiting signature data to render: ${this.value}`
      );
      var img = new Image();
      var self = this;
      img.onload = () =>
        self.canvasEl
          .getContext('2d')
          .drawImage(img, 0, 0, self.width, self.height);
      img.src = this.value;
    }
  }
  ...
  private setIsCapturing(isCapturing: boolean) {
    if (this.isEditable()) {
      ...
      this.logger.debug(
        `Signature component is currently ${
          this.isCapturing ? '' : 'not '
        }capturing.`
      );
    }
  }
  ...
  getUpdatedSignature() {
    const callExternalFn = eval(
      this.element.config.uiStyles.attributes.scriptName
    );
